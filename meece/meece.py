...
def plot_edges(node_coords, edges, axes, filename):
...
def plot_frame(frame, filename):
    ...
    for instance in filename['tracks'][:]:
        ...
        for i, (x_coord, y_coord) in enumerate(coords):
            ...
            if node not in node_coords:
                ...
                raise Exception("duplicate node? what do? exit and start debugging!")

        # plot edges
        plot_edges(node_coords, edges, ax, filename)
